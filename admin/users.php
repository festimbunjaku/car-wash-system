    ...
    <div class="flex flex-col justify-between flex-1 mt-6">
        <nav class="-mx-3 space-y-6 ">
            <div class="space-y-3 ">
                ...
                <!-- My Bookings Link -->
                <a class="flex items-center px-3 py-2 text-gray-600 transition-colors duration-300 transform rounded-lg dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 hover:text-gray-700 border bg-slate-800" href="./users.php">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
if (isset($_GET['delete_user_id'])) {
    $delete_user_id = filter_input(INPUT_GET, 'delete_user_id', FILTER_VALIDATE_INT);

    if ($delete_user_id) {
        // Delete the user, but keep their bookings in the database
        $sql = "DELETE FROM users WHERE id = :user_id";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':user_id' => $delete_user_id])) {
...
<section class="container mt-8 px-4 mx-auto w-full h-full">
    <div class="flex items-center gap-x-3">
        ...
        <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full"><?= count($users) ?> Users</span>
    </div>
                ...
                <div class="overflow-hidden border border-gray-200 md:rounded-lg">
                    <?php if (count($users) > 0): ?>
                        <table class="min-w-full divide-y divide-gray-200 mt-4">
                            <thead>
                                <tr>
                                    <th class="py-3.5 px-4 text-sm text-gray-500 text-center">ID</th>
                                    <th class="py-3.5 px-4 text-sm text-gray-500 text-center">Full Name</th>
                                    <th class="py-3.5 px-4 text-sm text-gray-500 text-center">Email</th>
                                    <th class="py-3.5 px-4 text-sm text-gray-500 text-center">Role</th>
                                    <th class="py-3.5 px-4 text-sm text-gray-500 text-center">Created At</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php foreach ($users as $user): ?>
                                    <tr>
                                        <!-- User ID -->
                                        <td class="px-4 py-4 text-sm text-gray-700 text-center"><?= $user['id'] ?></td>

                                        <!-- Full Name -->
                                        <td class="px-4 py-4 text-sm text-gray-700 text-center"><?= $user['fullname'] ?></td>

                                        <!-- Email -->
                                        <td class="px-4 py-4 text-sm text-gray-700 text-center"><?= $user['email'] ?></td>

                                        <!-- Role -->
                                        <td class="px-4 py-4 text-sm text-gray-700 text-center"><?= ucfirst($user['role']) ?></td>

                                        <!-- Created At -->
                                        <td class="px-4 py-4 text-sm text-gray-500 text-center"><?= date("m/d/Y H:i", strtotime($user['created_at'])) ?></td>

                                        <!-- Actions -->
                                        <td class="px-4 py-4 text-sm text-center">
                                            <a href="?delete_user_id=<?= $user['id'] ?>" class="text-gray-500 hover:text-red-500">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
